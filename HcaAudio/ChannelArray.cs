...
namespace DereTore.Exchange.Audio.HCA
{
    internal sealed unsafe class ChannelArray : DisposableBase
    {
        ...
        public void Decode1(int channelIndex, DataBits data, uint a, int b, byte[] ath)
        {
            ...
            if (v >= 6)
            {
                for (var i = 0; i < *pCount; ++i) pValue[i] = (sbyte) data.GetBit(6);
            }
            else if (v != 0)
            {
            ...
            {
                for (var i = 0; i < a; ++i) (*ppValue3)[i] = (sbyte) data.GetBit(6);
            }
        ...
        public void Decode5(int channelIndex, int index)
        {
            ...
            fixed (float* list3FloatBase = ChannelTables.Decode5List3Single)
            {
                ...
                for (var i = 0; i < 0x40; ++i) *d++ = *s1++ * *s++ + *s2++;

                for (var i = 0; i < 0x40; ++i) *d++ = *s++ * *--s1 - *s2++;

                s1 = &GetPtrOfWav2(channelIndex)[0x40 - 1];
                s2 = GetPtrOfWav3(channelIndex);

                for (var i = 0; i < 0x40; ++i) *s2++ = *s1-- * *--s;

                for (var i = 0; i < 0x40; ++i) *s2++ = *--s * *++s1;
            }
        }
        ...
        {
            if (_basePtr != IntPtr.Zero) Marshal.FreeHGlobal(_basePtr);
        ...
        {
            if (ptr == null || byteCount <= 0) return;

            var p = (byte*) ptr;

            for (var i = 0; i < byteCount; ++i) p[i] = 0;
        }
