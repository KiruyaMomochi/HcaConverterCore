...
namespace DereTore.Exchange.Audio.HCA
{
    internal sealed class Cipher
    {
        ...
        public void Decrypt(byte[] data, int length)
        {
            ...
            for (var i = 0; i < length; ++i) data[i] = _decryptTable[data[i]];
        }
        ...
        {
            length = Math.Min(data.Length, length);
            for (var i = 0; i < length; ++i) data[i] = _encryptTable[data[i]];
        }

        private bool Initialize(CipherType type, uint key1, uint key2)
        {
            if (key1 == 0 && key2 == 0 && type == CipherType.CipherWithAKey) type = CipherType.NoChipher;
        ...
        private void Init0()
        {
            for (var i = 0; i < _decryptTable.Length; ++i) _decryptTable[i] = (byte) i;
        }
        ...
        private void Init56(uint key1, uint key2)
        {
            ...
            var t3 = new byte[0x100];
            var t31 = new byte[0x10];
            var t32 = new byte[0x10];
            var c = 0;
            Init56CreateTable(t31, t1[0]);
            for (var i = 0; i < t31.Length; ++i)
            {
                Init56CreateTable(t32, t2[i]);
                var v = (byte) (t31[i] << 4);
                for (var j = 0; j < t32.Length; ++j)
                {
                    t3[c] = (byte) (v | t32[j]);
                    ++c;
                }
        ...
        {
            for (var i = 0; i < 0x100; ++i) _encryptTable[_decryptTable[i]] = (byte) i;
        }
