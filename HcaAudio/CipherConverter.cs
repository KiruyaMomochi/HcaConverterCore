    ...
    public sealed class CipherConverter : HcaReader
    {
        ...
        private void UpdateHeader()
        {
            ...
            if (MagicValues.IsMagicMatch(v, MagicValues.CIPH))
            {
                ...
                if (BitConverter.IsLittleEndian) u = DereToreHelper.SwapEndian(u);
        ...
        private void ConvertData()
        {
            ...
            for (var i = 0; i < totalBlockCount; ++i)
            {
                ...
                if (read < buffer.Length) throw new IOException("Something went wrong while trying to read data.");
                var r = ConvertBlock(buffer, outputStream);
                if (!r) throw new HcaException("Block conversion failed.", ActionResult.DecodeFailed);
            }
        }
        ...
        private static void FixDataBlock(byte[] blockData)
        {
            ...
            if (BitConverter.IsLittleEndian) sum = DereToreHelper.SwapEndian(sum);
        ...
        private static void OrBytes(byte[] blockData, int offset, int length)
        {
            ...
            for (var i = offset; i < blockData.Length && i < end; ++i) blockData[i] = (byte) (blockData[i] & 0xf0);
        }

        private void ProcessHeaderSignature(Stream stream, byte[] headerData)
        {
            if (!EncryptHeaderSignatures) return;
