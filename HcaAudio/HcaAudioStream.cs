...
namespace DereTore.Exchange.Audio.HCA
{
    public sealed class HcaAudioStream : HcaAudioStreamBase
    {
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            var length = Length;
            count = HasLoop && audioParams.InfiniteLoop
                ? Math.Min(count, buffer.Length - offset)
                : Math.Min((int) (length - position), count);
            ...
            while (count > 0)
            {
                ...
                var shouldRead = Math.Min(count,
                    waveBlockSize - (int) (positionNonLooped - headerSize) % waveBlockSize);
        ...
        public override long Length
        {
            get
            {
                ...
                var hcaInfo = HcaInfo;
                long totalLength = 0;
                if (_audioParams.OutputWaveHeader) totalLength += _headerSize;
        ...
        {
            get => EnsureNotDisposed() ? _position : 0;
            set
            {
                EnsureNotDisposed();
        ...
        public override float LengthInSeconds =>
            HcaHelper.CalculateLengthInSeconds(HcaInfo, _audioParams.SimulatedLoopCount, _audioParams.InfiniteLoop);

        public override uint LengthInSamples =>
            HcaHelper.CalculateLengthInSamples(HcaInfo, _audioParams.SimulatedLoopCount, _audioParams.InfiniteLoop);

        protected override void Dispose(bool disposing)
        {
            if (!IsDisposed) _memoryCache.Dispose();
        ...
        private void EnsureSoundDataDecoded(long waveDataOffset, int byteCount)
        {
            ...
            for (var i = startBlockIndex; i <= endBlockIndex; ++i) EnsureDecodeOneBlock(i);
        }
        ...
        private void EnsureSoundDataDecodedWithLoops(long waveDataOffset, int byteCount)
        {
            ...
            if (audioParams.InfiniteLoop || startBlockIndex <= hcaInfo.LoopEnd +
                (hcaInfo.LoopEnd - hcaInfo.LoopStart + 1) * audioParams.SimulatedLoopCount)
            {
                ...
                while (true)
                {
                    ...
                    if (!audioParams.InfiniteLoop && decodedSize >= waveDataEnd) break;
                    EnsureDecodeOneBlock(blockIndex);
                    decodedSize += waveBlockSize;
                    ++blockIndex;
                    if (blockIndex > hcaInfo.LoopEnd) blockIndex = hcaInfo.LoopStart;
                    executed = true;
                }
            }

            // And those following it.
            if (audioParams.InfiniteLoop) return;
            var endBlockIndex = hcaInfo.LoopEnd +
                                (hcaInfo.LoopEnd - hcaInfo.LoopStart + 1) * audioParams.SimulatedLoopCount;
            var startOfAfterLoopRegion = endBlockIndex * waveBlockSize;
            if (waveDataEnd >= startOfAfterLoopRegion)
                for (var blockIndex = hcaInfo.LoopEnd + 1;
                    blockIndex < hcaInfo.BlockCount && decodedSize < waveDataEnd;
                    ++blockIndex)
                {
                    EnsureDecodeOneBlock(blockIndex);
                    decodedSize += waveBlockSize;
        ...
        private void EnsureDecodeOneBlock(uint blockIndex)
        {
            ...
            if (decodedBlocks.ContainsKey(blockIndex)) return;
        ...
        private int GetTotalAfterDecodingWaveDataSize()
        {
            var hcaInfo = HcaInfo;
            var totalLength = 0;
            if (_audioParams.OutputWaveHeader) totalLength += _decoder.GetMinWaveHeaderBufferSize();
