...
namespace DereTore.Exchange.Audio.HCA
{
    partial class HcaDecoder
    {
        ...
        {
            if (waveDataBuffer == null) throw new ArgumentNullException(nameof(waveDataBuffer));
            var waveBlockSize = GetMinWaveDataBufferSize();
            if (waveDataBuffer.Length < waveBlockSize)
                throw new HcaException(ErrorMessages.GetBufferTooSmall(waveBlockSize, waveDataBuffer.Length),
        ...
        internal uint DecodeBlocks(byte[] waveDataBuffer, uint startBlockIndex)
        {
            ...
            if (numBlocksToDecode == 0) return 0;
            TransformWaveDataBlocks(SourceStream, waveDataBuffer, startBlockIndex, numBlocksToDecode,
                GetProperWaveWriter());
