...
namespace DereTore.Exchange.Audio.HCA
{
    partial class HcaDecoder
    {
        ...
        private void InitializeDecodeComponents()
        {
            ...
            var cipherType = decodeParams.CipherTypeOverrideEnabled
                ? decodeParams.OverriddenCipherType
                : hcaInfo.CipherType;
            ...
            var r = new byte[10];
            var b = hcaInfo.ChannelCount / hcaInfo.CompR03;
        ...
        private int DecodeToWaveR32(byte[] blockData, int blockIndex)
        {
            ...
            if (blockData.Length < hcaInfo.BlockSize)
                throw new HcaException(
                    ErrorMessages.GetInvalidParameter(nameof(blockData) + "." + nameof(blockData.Length)),
            ...
            try
            {
                ...
                for (i = 0; i < 8; ++i)
                {
                    ...
                    for (var j = 0; j < hcaInfo.ChannelCount; ++j)
                    {
                        ...
                        channels.Decode3(j, hcaInfo.CompR09, hcaInfo.CompR08,
                            (uint) (hcaInfo.CompR07 + hcaInfo.CompR06), hcaInfo.CompR05);
                    }

                    for (var j = 0; j < hcaInfo.ChannelCount - 1; ++j)
                    {
                        site = $"Decode4({i.ToString()}/{j.ToString()})";
                        channels.Decode4(j, j + 1, i, (uint) (hcaInfo.CompR05 - hcaInfo.CompR06), hcaInfo.CompR06,
                            hcaInfo.CompR07);
                    }
            ...
            catch (IndexOutOfRangeException ex)
            {
                const string message =
                    "Index access exception detected. It is probably because you are using an incorrect HCA key pair while decoding a type 56 HCA file.";
        ...
        private void TransformWaveDataBlocks(Stream source, byte[] destination, uint startBlockIndex, uint blockCount,
            IWaveWriter waveWriter)
        {
            if (source == null) throw new ArgumentNullException(nameof(source));
            if (destination == null) throw new ArgumentNullException(nameof(destination));
            if (waveWriter == null) throw new ArgumentNullException(nameof(waveWriter));
            var hcaInfo = HcaInfo;
