...
namespace DereTore.Exchange.Audio.HCA
{
    public partial class HcaDecoder : HcaReader, IDisposable
    {
        ...
        {
            if (_minWaveHeaderBufferSize != null) return _minWaveHeaderBufferSize.Value;
            var wavNoteSize = 0;
            var hcaInfo = HcaInfo;
            if (hcaInfo.Comment != null)
            {
                wavNoteSize = 4 + (int) hcaInfo.CommentLength + 1;
                if ((wavNoteSize & 3) != 0) wavNoteSize += 4 - (wavNoteSize & 3);
            }

            var sizeNeeded = Marshal.SizeOf(typeof(WaveRiffSection));
            if (hcaInfo.LoopFlag) sizeNeeded += Marshal.SizeOf(typeof(WaveSampleSection));
            if (hcaInfo.Comment != null && hcaInfo.Comment.Length > 0) sizeNeeded += 8 * wavNoteSize;
        ...
        {
            if (_minWaveDataBufferSize != null) return _minWaveDataBufferSize.Value;
        ...
        public int WriteWaveHeader(byte[] stream, AudioParams audioParams)
        {
            ...
            var hcaInfo = HcaInfo;
            if (hcaInfo.LoopFlag && audioParams.InfiniteLoop) // See remarks of AudioParams.InfiniteLoop.
                throw new HcaException(ErrorMessages.GetInvalidParameter(nameof(audioParams.InfiniteLoop)),
                    ActionResult.InvalidParameter);
            var minimumHeaderBufferSize = GetMinWaveHeaderBufferSize();
            if (stream.Length < minimumHeaderBufferSize)
                throw new HcaException(ErrorMessages.GetBufferTooSmall(minimumHeaderBufferSize, stream.Length),
                    ActionResult.BufferTooSmall);
            ...
            if (hcaInfo.Comment != null)
            {
                ...
                if ((wavNote.NoteSize & 3) != 0) wavNote.NoteSize += 4 - (wavNote.NoteSize & 3);
            }
            ...
            wavRiff.RiffSize = (uint) (0x1c + (hcaInfo.Comment != null ? wavNote.NoteSize : 0) +
