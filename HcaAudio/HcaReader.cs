        ...
        internal void ParseHeaders()
        {
            ...
            if (MagicValues.IsMagicMatch(v, MagicValues.FMT))
            {
                ...
                if (hcaInfo.ChannelCount < 1 || hcaInfo.ChannelCount > 16)
                    throw new HcaException($"Channel count should be between 1 and 16, read {hcaInfo.ChannelCount}.",
                        ActionResult.InvalidFieldValue);
                if (hcaInfo.SamplingRate < 1 || hcaInfo.SamplingRate > 0x7fffff)
                    throw new HcaException(
                        $"Sampling rate should be between 1 and {0x7fffffff}, read {hcaInfo.SamplingRate}.",
                        ActionResult.InvalidFieldValue);
            }
            else
            {
            ...
            if (MagicValues.IsMagicMatch(v, MagicValues.COMP))
            {
                ...
                if ((hcaInfo.BlockSize < 8 || hcaInfo.BlockSize > 0xffff) && hcaInfo.BlockSize != 0)
                    throw new HcaException($"Block size should be between 8 and {0xffff}, read {hcaInfo.BlockSize}.",
                        ActionResult.InvalidFieldValue);
                if (!(hcaInfo.CompR01 <= hcaInfo.CompR02 && hcaInfo.CompR02 <= 0x1f))
                    throw new HcaException(
                        $"CompR01 should be less than or equal to CompR02, and CompR02 should be less than or equal to {0x1f}, read {hcaInfo.CompR01} and {hcaInfo.CompR02}.",
                        ActionResult.InvalidFieldValue);
            }
            else if (MagicValues.IsMagicMatch(v, MagicValues.DEC))
            {
                ...
                if (!(hcaInfo.CompR01 <= hcaInfo.CompR02 && hcaInfo.CompR02 <= 0x1f))
                    throw new HcaException(
                        $"CompR01 should be less than or equal to CompR02, and CompR02 should be less than or equal to {0x1f}, read {hcaInfo.CompR01} and {hcaInfo.CompR02}.",
                        ActionResult.InvalidFieldValue);
                if (hcaInfo.CompR03 == 0) hcaInfo.CompR03 = 1;
            }
            else
            {
            ...
            if (MagicValues.IsMagicMatch(v, MagicValues.VBR))
            {
                ...
                if (!(hcaInfo.BlockSize == 0 && hcaInfo.VbrR01 < 0x01ff))
                    throw new HcaException($"VbrR01 should be less than {0x01ff} in VBR HCA.",
                        ActionResult.InvalidFieldValue);
            }
            else
            {
            ...
            // PAD (undocumented)
            v = stream.PeekUInt32LE();
            if (MagicValues.IsMagicMatch(v, MagicValues.PAD)) stream.Skip(4); // Length of 'pad '

            if (hcaInfo.CompR03 == 0) hcaInfo.CompR03 = 1;

            if (hcaInfo.CompR01 != 1 || hcaInfo.CompR02 != 0xf)
                throw new HcaException($"Expected CompR01=1, CompR02=15, read {hcaInfo.CompR01}, {hcaInfo.CompR02}.",
                    ActionResult.InvalidFieldValue);
            hcaInfo.CompR09 = HcaHelper.Ceil2((uint) (hcaInfo.CompR05 - (hcaInfo.CompR06 + hcaInfo.CompR07)),
                hcaInfo.CompR08);
            HcaInfo = hcaInfo;
        }
